...
# ── Anthropic API Key ──────────────────────────────────────────────────────────
ANTHROPIC_API_KEY = "YOUR_ANTHROPIC_API_KEY_HERE"   # ← replace with your real key
# ──────────────────────────────────────────────────────────────────────────────
...
def chat():
    ...
    system_prompt = (
        ...
        "PetNova's pages: / (home), /pets (browse all), /buy (buy pets), /sell (sell/list a pet), "
        "/mate (find breeding partner), /rescue (lost & found / rescue), /vet (vet directory), "
        "/petaccessories (pet shops), /care-tips (personalized care tips), /quiz (pet quizzes), /add (add a pet). "
        "Keep answers concise, warm, and helpful. Use pet-related emojis occasionally. "
        "If asked about something unrelated to pets or PetNova, politely redirect the conversation back to pets."
    )
    ...
    try:
        ...
        return jsonify({"reply": f"Sorry, I'm having trouble right now. Please try again! 🐾"}), 500
# ══════════════════════════════════════════════════════════════════════════════
